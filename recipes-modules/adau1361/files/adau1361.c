...
MODULE_AUTHOR
    ("Xilinx Inc.");
MODULE_DESCRIPTION
    ("adau1361 - loadable module template generated by petalinux-create -t modules");
...
static irqreturn_t adau1361_irq(int irq, void *lp)
{
	printk("adau1361 interrupt\n");
	return IRQ_HANDLED;
}
...
static int adau1361_probe(struct platform_device *pdev)
{
	...
	if (!r_irq) {
		...
		dev_info(dev, "adau1361 at 0x%08x mapped to 0x%08x\n",
			(unsigned int __force)lp->mem_start,
			(unsigned int __force)lp->base_addr);
		return 0;
	}
	lp->irq = r_irq->start;
	rc = request_irq(lp->irq, &adau1361_irq, 0, DRIVER_NAME, lp);
	if (rc) {
...
static struct of_device_id adau1361_of_match[] = {
	{ .compatible = "vendor,adau1361", },
	{ /* end of list */ },
};
MODULE_DEVICE_TABLE(of, adau1361_of_match);
#else
# define adau1361_of_match
#endif
